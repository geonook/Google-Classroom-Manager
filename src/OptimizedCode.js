...
function listCoursesUI() {
  ...
  const result = ui.prompt('請輸入工作表名稱', 
...
function createCoursesUI() {
  ...
  const sheetNameResult = ui.prompt('步驟 1/2: 請輸入工作表名稱',
  ...
  const ownerIdResult = ui.prompt('步驟 2/2: 請輸入課程擁有者 ID',
...
function addMembersUIOptimized(role) {
  ...
  const defaultSheetName = getScriptProperty(`defaultSheet_add${role}s`) || 
    (role === 'TEACHER' ? '新增課程與老師' : '新增課程與學生');
    
  const result = ui.prompt(`新增${roleText}`,
...
async function listCourseMembersOptimized(sheetName) {
  ...
  if (checkedCourses.length === 0) {
    SpreadsheetApp.getUi().alert('提示', '沒有找到已勾選的課程', SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
...
async function createCoursesOptimized(sheetName, ownerId) {
  ...
  if (coursesToCreate.length === 0) {
    SpreadsheetApp.getUi().alert('提示', '沒有找到待建立的課程', SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
...
async function addMembersOptimized(sheetName, role) {
  ...
  if (membersToAdd.length === 0) {
    SpreadsheetApp.getUi().alert('提示', `沒有找到待新增的${role === 'TEACHER' ? '老師' : '學生'}`, SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }

...
class AICourseClassifier {
  ...
  analyzeCourse(courseName, confidenceThreshold = 0.7) {
    ...
    // 判斷是否通過信心度門檻
    const success =
      analysis.confidence.overall >= confidenceThreshold &&
      analysis.grade &&
      analysis.className &&
      analysis.subject;
...
class AIMappingValidator {
  ...
  validateMappings(mappings) {
    ...
    console.log(
      `✅ 驗證完成 - 通過: ${validation.passed}, 警告: ${validation.warnings}, 錯誤: ${validation.errors}`
    );
  ...
  checkCompleteness(mappings) {
    ...
    const actualCombinations = new Set(mappings.map((m) => `${m.courseName}-${m.subject}`));
  ...
  calculateStatistics(mappings) {
    ...
    mappings.forEach((mapping) => {
      ...
      // 狀態統計
      stats.byStatus[mapping.status || 'UNKNOWN'] =
        (stats.byStatus[mapping.status || 'UNKNOWN'] || 0) + 1;
...
class AIMissingCoursePredictor {
  ...
  predictMissingCourses(expectedCombinations, actualMappings, availableCourses) {
    ...
    const actualCombinations = new Set(actualMappings.map((m) => `${m.courseName}-${m.subject}`));
...
class AIMappingOptimizer {
  ...
  async optimizeMapping(courses, options = {}) {
    ...
    try {
      ...
      console.log(
        `📊 最終結果: ${results.mappings.length} 個映射, ${results.improvements.length} 項改進`
      );
  ...
  async classifyAllCourses(courses, strategy) {
    ...
    return {
      ...
      statistics: {
        ...
        averageConfidence:
          mappings.reduce((sum, m) => sum + m.score, 0) / Math.max(1, mappings.length),
      },
    };
  }
...
async function cleanAndStandardizeMappingData() {
  ...
  try {
    ...
    console.log(
      `📊 清理統計: ${cleaningStats.totalCleaned}/${cleaningStats.totalProcessed} 個映射已標準化`
    );
        ...
        switch (issue.type) {
          ...
          case 'MISSING_MAPPING':
            ...
            if (courses.success) {
              ...
              const prediction = predictor.predictAlternatives(
                className,
                subject,
                courses.courses,
                1
              ...
              if (
                ...
                prediction.alternatives[0].matchScore > 0.7
              ) {
                const newMapping = {
